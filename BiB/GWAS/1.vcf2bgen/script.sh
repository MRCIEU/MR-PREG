...
if [ ${PBS_ARRAYID} -gt 9 ]
then 
	data_file=data_chr0${PBS_ARRAYID}
else 
	data_file=data_chr0${PBS_ARRAYID}
fi

qctool -g path/to/${data_file}.vcf.gz -og path/to/${data_file}_bib.bgen -os path/to/sampleid_bib0${PBS_ARRAYID}.sample -sex-column sex
