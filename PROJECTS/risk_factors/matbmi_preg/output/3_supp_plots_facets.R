...
#remove the fixed/random effects from the plot
mv.all_res_nopool<-subset(mv.all_res, study!="Pooled (FE)")
mv.all_res_nopool<-subset(mv.all_res_nopool, study!="Pooled (RE)")
...
df1a <- sel_dat(mv.all_res_nopool, unique(all_res$study), "adjusted", "binary") %>% filter(group %in% c("Pregnancy", "Delivery"))

fplot_dat(df1a, df1a$study, or = T) + scale_color_manual(values=color)

ggsave(paste0(out_dir, "forest_mv-studies_bin_part1_suppfig4A.jpg"),  width = 15, height = 10)

# Binary traits - part 1

df1b <- sel_dat(mv.all_res_nopool, unique(all_res$study), "adjusted", "binary") %>% filter(group %in% c("Birth", "Postnatal"))

fplot_dat(df1b, df1b$study, or = T) + scale_color_manual(values=color)

ggsave(paste0(out_dir, "forest_mv-studies_bin_part2_suppfig4A.jpg"),  width = 15, height = 10)

# Continuous traits 

df1c <- sel_dat(mv.all_res_nopool, unique(all_res$study), "adjusted", "continuous")

fplot_dat(df1c, df1c$study, or = F) + xlab("MD (95% CI)\nper 1-SD increment in BMI") + scale_color_manual(values=color)

ggsave(paste0(out_dir, "forest_mv-studies_cont_suppfig4B.jpg"),  width = 15, height = 10)
