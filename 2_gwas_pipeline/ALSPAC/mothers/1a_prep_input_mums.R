...
phen.dir <- paste0(Sys.getenv('MRPREG_sumdat'), "data/GWAS/ALSPAC/mothers/")
pcs.dir  <- paste0(Sys.getenv('ALSPAC_GENO'), "/derived/principal_components/plink/mothers/")
out.dir <- paste0(Sys.getenv('MRPREG_sumdat'), "data/GWAS/ALSPAC/mothers/")
...
# Outcomes
write.table(plink_phen, file = paste0(out.dir, "alspac_mum.phen"), quote = F, sep = " ", row.names = F)
...
# Sample files
write.table(samp, file = paste0(out.dir, "alspac_mums.sample"), quote = F, row.names = F)	

# Covariables
write.table(covar, file = paste0(out.dir, "alspac_mums.pcs"), quote = F, row.names = F)
