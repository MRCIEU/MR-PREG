...
prefix=BIB.${traitname}.${personC}.${ethnicityE}
echo ${traitname}
...
for i in {1..22}; do
  ...
  --covarFile ${dat}/pheno.we.master.children.txt \
  --phenoFile ${dat}/pheno.we.master.children.txt \
  --phenoCol ${traitname} \
  ...
  --minMAC 20 \
  --threads 10 \
  --pred  ${modelsOffWE}/${prefix}_step1_pred.list \
  --out ${resOffWE}/${prefix}_chr${i}
done
