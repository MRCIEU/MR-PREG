...
varlist_fpath <- paste0(sumdat_dir, "Metanalysis/offspring/", "vars_metanalysis_v5_child_final.txt")
...
comb_info <- function(x) {
  ...
  if(!is.na(varlist$var_MOBA[x])){
    df_moba <- extr_info(
	  index = x,
      study = "MOBA",
      rel_path = "MOBA/offspring/final",
      prefix = paste0("^moba_100k_child_", varlist$var_MOBA[x])
	) 
	df <- df_moba
  }
  ...
  if(!is.na(varlist$var_ALSPAC[x])){
    df_alspac <- extr_info(
	  index = x,
      study = "ALSPAC",
      rel_path = "ALSPAC/offspring/final",
      prefix = paste0("^alspac_child_", varlist$var_ALSPAC[x])
	) 
  ...
  if(!is.na(varlist$var_BIB.WE[x])){
    df_bib.we <- extr_info(
	  index = x,
      study = "BIB.WE",
      rel_path = "BIB-WE/offspring",
      prefix = paste0("^BIB_child.european_", varlist$var_BIB.WE[x], "_2024"),
	  strata = "WE"
	) 
  ...
  if(!is.na(varlist$var_BIB.SA[x])){
    df_bib.sa <- extr_info(
	  index = x,
      study = "BIB.SA",
      rel_path = "BIB-SA/offspring",
      prefix = paste0("^BIB_child.southasian_", varlist$var_BIB.SA[x], "_2024"),
	  strata = "SA"	
	) 
  ...
  if(!is.na(varlist$var_UKB[x])){
    df_ukb <- extr_info(
	  index = x,
      study = "UKB",
      rel_path = "UKB/offspring/final",
      prefix = paste0("^ukb_child_", varlist$var_UKB[x], "_2024")
	) 
  ...
  if(!is.na(varlist$var_GWAS[x])){
    if(varlist$var_GWAS[x] %in% c("ga_subsamp", "pretb_subsamp", "posttb_subsamp", "zbw_subsamp")) {
	   consortium <- "EGG"
    ...
    df_gwas <- extr_info(
	  index = x,
      study = "GWAS",
      rel_path = "Public/offspring",
      prefix = paste0("^", consortium, "_child_", varlist$var_GWAS[x])
	) 
...
for (i in 1:nrow(varlist)){
  ...
  ## Import METAL file
	metal_output <- list.files(
	  path = paste0(sumdat_dir, "Metanalysis/offspring/METAL_original_output/"), 
  ...
  ## Keep if association estimated in > 50% ncase/sample size
	...
	if(pheno_name %in% bin.out) {
	  metal_comb <- filter(metal_comb, ncase > max(metal_comb$ncase)/2)
	} else if(pheno_name %in% cont.out) {
	  metal_comb <- filter(metal_comb, samplesize > max(metal_comb$samplesize)/2)
	}
  ...
  ## Save clean files
    write.table(metal_comb, file = gzfile(
	  paste0(sumdat_dir, "Metanalysis/offspring/final/", "metanalyses-R3.child.", varlist$Variable_name[i], ".txt.gz")), 
